...
int main(void)
{
	...
	struct sockaddr_in my_clients[10];
    int n = 0;
    ...
    while (1)
    {
        ...
        for (i = 0; i < n; i++)
        {
            ...
            for (j = 0; j < n; j++)
            {
                ...
                if (sendto(s, &my_clients[j], sizeof(struct sockaddr_in), 0, (struct sockaddr*)(&si_other), slen)==-1)
                    diep("sendto");
            }
        }
